package com.pinhost.mm.dtcdatabase.service;
...
public class UserLoginService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        UserModel userModel = this.userRepositoryH2.getUserByName(username);
        if (userModel != null)
        {
    ...
    private UserDetails buildUser(UserModel user, List<GrantedAuthority> authorities)
    {
    ...
    private UserModel getUserWithRoles(UserModel user){
        List<RoleModel> userRoles = new ArrayList<>();
        List<UserModel> userList = userRepositoryH2.getUserModelListByName(user.getName());
        if (!userList.isEmpty()){
